---
created: 2026-03-27
tags:
  - diary
  - electronics
---
Today i wanted to talk about how i recently got into automation. I have a "server", which is just a laptop without a screen, glued on the underside of my desk (i know, i know), linked to my network using a 10/100 24 port switch that a friend of mine generously gave to me (also glued to my desk).
![[Pasted image 20260327092150.png]]i had quite some time to think about what to host in this server, and i came up with the idea to host a homeassistnat instance using docker and not the official immage, and so i did. I later integrate the very few smart devices i have in my home (2 smart lights and a smart socket). But that wasn't enough for me since i 